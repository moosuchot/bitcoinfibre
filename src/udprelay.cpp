...
static void FillBlockMessageHeader(UDPMessage& msg, const uint64_t hash_prefix, UDPMessageType type, const std::vector<unsigned char>& data) {
...
void UDPRelayBlock(const CBlock& block) {
    ...
    if (maybe_have_write_nodes) { // Scope for partial_block_lock and partial_block_ptr
        ...
        uint64_t target_mbps = min_group_mbps.load(std::memory_order_relaxed);
        uint64_t target_nodes = std::max(node_count.load(std::memory_order_relaxed), uint64_t(1));
        DataFECer header_fecer(data, (target_mbps * 1024 * 1024 / 8 / 1000 / PACKET_SIZE / target_nodes) + 10); // 1ms + 10 chunks of header FEC
        ...
        if (fBench) {
            ...
            LogPrintf("UDP: Built all FEC chunks for block %s in %lf %lf %lf %lf %lf %lf %lf ms with %lu (%lu/%lu) header chunks\n", hashBlock.ToString(), to_millis_double(initd - start), to_millis_double(coded - initd), to_millis_double(feced - coded), to_millis_double(header_sent - feced), to_millis_double(block_coded - header_sent), to_millis_double(block_fec_initd - block_coded), to_millis_double(all_sent - block_fec_initd), header_fecer.fec_chunks, target_mbps, target_nodes);
            if (!inUDPProcess)
