...
struct DataFECer {
    ...
    DataFECer(FECDecoder&& decoder, const std::vector<unsigned char>& data, int32_t of_prng_seed, size_t fec_chunks_in) :
        fec_chunks(fec_chunks_in),
        fec_data(fec_chunks * FEC_CHUNK_SIZE),
        enc(std::move(decoder), &data, of_prng_seed, &fec_data) {}
};
...
void UDPRelayBlock(const CBlock& block) {
    ...
    { // Scope for partial_block_lock and partial_block_ptr
        ...
        if (inUDPProcess) {
            ...
            if (!block_chunks->empty()) {
                ...
                if (skipEncode) {
                    ...
                    new (block_fecer) DataFECer(std::move(partial_block_ptr->decoder), *block_chunks, of_prng_seed, data_fec_chunks);
                } else {
