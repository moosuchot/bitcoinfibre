...
struct PartialBlockData {
    int64_t  timeHeaderRecvd;
    bool currentlyProcessing = false; // Prevents timeout for trusted sources
    CService nodeHeaderRecvd;
    uint32_t obj_length; // FEC-coded length of currently-being-download object
    uint32_t chunks_sent;
    bool in_header; // Indicates we are currently downloading header (or block txn)
    bool initialized; // Indicates Init has been called
...
static std::map<std::pair<uint64_t, CService>, PartialBlockData> mapPartialBlocks;
static std::set<uint64_t> setBlocksRelayed;

static std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator RemovePartialBlock(std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator it) {
...
static void RemovePartialBlocks(uint64_t hash_prefix) {
    std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator it = mapPartialBlocks.lower_bound(std::make_pair(hash_prefix, TRUSTED_PEER_DUMMY));
...
static void SendFECData(UDPMessage& msg, const std::vector<unsigned char>& data, size_t fec_chunks, int32_t of_prng_seed, uint64_t hash_prefix) {
    const size_t msg_chunks = DIV_CEIL(data.size(), FEC_CHUNK_SIZE);

    std::vector<unsigned char> fec_data(fec_chunks * FEC_CHUNK_SIZE);
    FECEncoder enc(&data, of_prng_seed, &fec_data);

    std::vector<size_t> order(fec_chunks);
    std::mt19937 g(GetTimeMicros());
    for (size_t i = 0; i < fec_chunks; i++)
        order[i] = i;
    std::shuffle(order.begin(), order.end(), g);

    for (auto it = mapUDPNodes.begin(); it != mapUDPNodes.end(); it++) {
        auto send_it = it;
        for (uint16_t i = 0; i < fec_chunks; i++) {
            assert(enc.BuildChunk(order[i])); // TODO: Handle errors?

            msg.msg.block.chunk_id = htole16(order[i] + msg_chunks);
            memcpy(msg.msg.block.data, &fec_data[order[i] * FEC_CHUNK_SIZE], FEC_CHUNK_SIZE);
...
static void SendFECedData(const uint256& blockhash, UDPMessageType type, const std::vector<unsigned char>& data, int32_t of_prng_seed) {
    const size_t msg_chunks = DIV_CEIL(data.size(), FEC_CHUNK_SIZE);
    size_t fec_chunks;
    if (type == MSG_TYPE_BLOCK_HEADER) // Send header data for 1 ms
        fec_chunks = (NETWORK_TARGET_BYTES_PER_SECOND / 1000 / PACKET_SIZE / mapUDPNodes.size()) + 10;
    else
        fec_chunks = msg_chunks + 10; //TODO: Pick something different?
    ...
    if (type == MSG_TYPE_BLOCK_HEADER) {
        ...
        SendFECData(msg, data, fec_chunks, of_prng_seed, hash_prefix);
    } else {
        SendFECData(msg, data, fec_chunks, of_prng_seed, hash_prefix);
        SendMessageData(msg, data, hash_prefix);
    }
...
static boost::thread *process_new_block_thread = NULL;
void UDPRelayBlock(const CBlock& block) {
    ...
    const std::vector<unsigned char> *block_chunks;
    bool skipEncode = false;
    std::unique_lock<std::recursive_mutex> lock(cs_mapUDPNodes, std::defer_lock);
    if (process_new_block_thread && boost::this_thread::get_id() == process_new_block_thread->get_id()) {
        lock.lock();

        auto it = mapPartialBlocks.find(std::make_pair(hash_prefix, TRUSTED_PEER_DUMMY));
        if (it != mapPartialBlocks.end() && it->second.currentlyProcessing && it->second.block_data.AreChunksAvailable()) {
            if (fBench)
    ...
    if (!skipEncode) {
        new (codedBlock) ChunkCodedBlock(block, true);
        block_chunks = &codedBlock->GetCodedBlock();
        stream << codedBlock->headerAndIDs;
    } else
        stream << CBlockHeaderAndLengthShortTxIDs(block, true);
    ...
    SendFECedData(hashBlock, MSG_TYPE_BLOCK_HEADER, data, of_prng_seed);

    // Now (maybe) send the transaction chunks
    if (!block_chunks->empty())
        SendFECedData(hashBlock, MSG_TYPE_BLOCK_CONTENTS, *block_chunks, of_prng_seed);

    if (fBench) {
        std::chrono::steady_clock::time_point all_sent(std::chrono::steady_clock::now());
        LogPrintf("UDP: Built all FEC chunks for block %s in %lf ms\n", hashBlock.ToString(), to_millis_double(all_sent - start));
...
static std::mutex new_block_mutex;
static std::condition_variable new_block_cv;
static std::atomic_bool new_block_available(false), block_process_shutdown(false);
static uint64_t block_currently_processing; // cs_mapUDPNodes || new_block_available
static CBlock decoded_block;
static void ProcessNewBlockThread() {
    while (true) {
        std::unique_lock<std::mutex> lock(new_block_mutex);
        new_block_available = false;
        while (!new_block_available && !block_process_shutdown)
            new_block_cv.wait(lock);
        ...
        if (!CheckBlock(decoded_block, validationstate, Params().GetConsensus(), GetAdjustedTime())) {
            ...
            RemovePartialBlocks(block_currently_processing);
        ...
        RemovePartialBlocks(block_currently_processing); // Ensure we remove even if we didnt UDPRelayBlock()
    }
}

static ReadStatus ProcessNewBlock(const uint64_t hash_prefix, PartialBlockData& block) {
    ...
    // Instead we have the ProcessNewBlockThread tell us when its running ProcessNewBlock and,
    // when it is, just fail to decode (should be sufficiently rare to finish decoding a block
    // while another is still being processed, so the failure rate generated by this is OK)
    assert(block.block_data.IsBlockAvailable());
    std::unique_lock<std::mutex> lock(new_block_mutex);
    if (new_block_available) {
        LogPrintf("UDP: Dropped potentially-decodeable block %s due to cs_main contention!\n", block.block_data.GetBlockHash().ToString());
        return READ_STATUS_FAILED;
    }
    ReadStatus status = block.block_data.GetBlock(decoded_block);

    new_block_available = true;
    block_currently_processing = hash_prefix;
...
static void BlockRecvInit() {
    process_new_block_thread = new boost::thread(boost::bind(&TraceThread<void (*)()>, "udpprocess", &ProcessNewBlockThread));
}

static void BlockRecvShutdown() {
    if (process_new_block_thread) {
        block_process_shutdown = true;
        new_block_cv.notify_all();
        process_new_block_thread->join();
        delete process_new_block_thread;
        process_new_block_thread = NULL;
    }
}
...
PartialBlockData::PartialBlockData(const CService& node, const UDPMessage& msg) :
        ...
        in_header(true), initialized(false), block_data(&mempool)
    { assert(Init(msg)); }
...
static bool HandleBlockMessage(UDPMessage& msg, size_t length, const CService& node, UDPConnectionState& state) {
    ...
    if (chunks_avail_it == state.chunks_avail.end()) {
        if (msg.header.msg_type == MSG_TYPE_BLOCK_HEADER) {
            if (state.chunks_avail.size() > 1 && !state.connection.fTrusted) {
                ...
                std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator first_partial_block_it  = mapPartialBlocks.find(std::make_pair(state.chunks_avail. begin()->first, node));
                assert(first_partial_block_it != mapPartialBlocks.end());
                std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator second_partial_block_it = mapPartialBlocks.find(std::make_pair(state.chunks_avail.rbegin()->first, node));
    ...
    std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator it = mapPartialBlocks.find(std::make_pair(hash_prefix, state.connection.fTrusted ? TRUSTED_PEER_DUMMY : node));
    if (it == mapPartialBlocks.end()) {
        if (msg.header.msg_type == MSG_TYPE_BLOCK_HEADER)
            it = mapPartialBlocks.insert(std::make_pair(std::make_pair(hash_prefix, state.connection.fTrusted ? TRUSTED_PEER_DUMMY : node), PartialBlockData(node, msg))).first;
        else // Probably stale (ie we just finished reconstructing)
            return true;
    }
    ...
    if (state.connection.fTrusted && block_currently_processing == hash_prefix) { // Must come before any chunk-accessors in block.block_data
    ...
    if (msg.header.msg_type == MSG_TYPE_BLOCK_CONTENTS && !block.initialized) {
        ...
        for (uint32_t i = 0; i < DIV_CEIL(block.obj_length, sizeof(UDPBlockMessage::data)); i++) {
            if (block.block_data.IsChunkAvailable(i)) {
                block.decoder.ProvideChunk(block.block_data.GetChunk(i), i);
                mempool_provided_chunks++;
            }
        }
        LogPrintf("UDP: Initialized block %s with %ld/%ld mempool-provided chunks\n", block.block_data.GetBlockHash().ToString(), mempool_provided_chunks, block.block_data.GetChunkCount());
    }
    ...
    if (block.decoder.DecodeReady()) {
        ...
        if (block.block_data.IsBlockAvailable()) {
            ...
            // from inside ProcessNewBlock, so after we notify the ProcessNewBlockThread we cannot access block.
            std::map<CService, std::pair<uint32_t, uint32_t> > chunksProvidedByNode;
            ...
            if (fBench) {
                ...
                decoded_block_hash = block.block_data.GetBlockHash(); // Cached for us
                origSender = block.nodeHeaderRecvd;
            }

            ReadStatus decode_status = ProcessNewBlock(hash_prefix, block);
            if (decode_status != READ_STATUS_OK) {
                RemovePartialBlock(it); // block is now erased - do not use
                if (decode_status == READ_STATUS_FAILED) {
                    LogPrintf("UDP: Failed to decode block from %s\n", node.ToString());
                    return true;
                } else {
                    LogPrintf("UDP: Got invalid block from %s\n", node.ToString());
                    return false;
                }
            } else {
                if (fBench) {
                    ...
                    LogPrintf("UDP: Block %s reconstructed from %s with %u chunks in %lf ms (%u recvd from %u peers)\n", decoded_block_hash.ToString(), origSender.ToString(), total_chunks_used, (GetTimeMicros() - timeBlockRecv) / 1000.0, total_chunks_recvd, chunksProvidedByNode.size());
                    for (const std::pair<CService, std::pair<uint32_t, uint32_t> >& provider : chunksProvidedByNode)
...
static void ProcessDownloadTimerEvents() {
    for (std::map<std::pair<uint64_t, CService>, PartialBlockData>::iterator it = mapPartialBlocks.begin(); it != mapPartialBlocks.end();) {
        if ((it->first.second != TRUSTED_PEER_DUMMY || !it->second.currentlyProcessing) && it->second.timeHeaderRecvd < GetTimeMicros() - 1000 * 1000 * 1000)
            it = RemovePartialBlock(it);
        else
            it++;
    }
...
static void timer_func(evutil_socket_t fd, short event, void* arg) {
    ...
    for (std::map<CService, UDPConnectionState>::iterator it = mapUDPNodes.begin(); it != mapUDPNodes.end(); it++) {
        ...
        if (state.lastRecvTime < now - 1000 * 60 * 10) {
            ...
            if (it != mapUDPNodes.begin())
                it--; // it will be incremented at end-of-loop
            continue;
        }
