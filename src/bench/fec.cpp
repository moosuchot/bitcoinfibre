...
void __attribute__((noinline)) DoRealFECedBlockRoundTripTest(benchmark::State& state, CTxMemPool& pool, CBlock& block) {
    size_t total_chunks_consumed, total_chunks_in_mempool, non_fec_chunks;
...
static void RealFECedBlockRoundTripTest(benchmark::State& state, int ntxn)
{
    ...
    DoRealFECedBlockRoundTripTest(state, pool, block);
}
