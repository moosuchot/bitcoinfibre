...
public:
    ...
    bool RecvBlockChunk(const unsigned char* chunk, size_t idx) {
        ...
        if (decoder->DecodeReady()) {
            ...
            CBlock block2;
            assert(partialBlock.GetBlock(block2) == READ_STATUS_OK);
            assert(block2.GetHash() == uint256S("0000000000000000025aff8be8a55df8f89c77296db6198f272d6577325d4069"));
            bool mutated;
            assert(block2.hashMerkleRoot == BlockMerkleRoot(block2, &mutated));
            assert(!mutated);
