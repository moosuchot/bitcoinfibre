...
public:
    const CBlockHeaderAndLengthShortTxIDs headerAndIDs;
    ChunkCodedBlock(const CBlock& block, bool fDeterministic = false);
...
public:
    ...
    ReadStatus GetBlock(CBlock& block) const;
    std::vector<unsigned char>& GetCodedBlock() { assert(AreChunksAvailable() && IsBlockAvailable()); return codedBlock; }
