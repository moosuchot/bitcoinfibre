...
UniValue getudppeerinfo(const UniValue& params, bool fHelp)
{
    ...
    for (const UDPConnectionStats& stats : vstats) {
        ...
        obj.push_back(Pair("avg_recent_rtt", total / stats.last_pings.size()));
...
UniValue disconnectudpnode(const UniValue& params, bool fHelp)
{
    string strCommand;
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "udpnetwork",         "disconnectudpnode",      &disconnectudpnode,      true  },
};
